...
export const ExamSetupModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 animate-fade-in bg-black/40 backdrop-blur-sm">
      <div className="bg-card w-full max-w-md rounded-t-[32px] sm:rounded-[32px] p-6 shadow-2xl border-t-2 sm:border-2 border-black/5 animate-sheet-up flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 mb-6">
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 gap-2">
                  <div className="px-4 py-3 rounded-[16px] bg-primary text-card font-bold text-sm shadow-brutal-sm">
                    {resolveExamType()}
                  </div>
                </div>
              </div>
              ...
              {isExit && profileStream && (
                <div className="space-y-2">
                  ...
                  <div className="px-4 py-3 rounded-[14px] bg-primary/5 border-2 border-primary/10 font-bold text-primary text-sm">
                    {profileStream}
                  </div>
                </div>
              )}
              ...
              <div className="space-y-2 pt-2">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button type="button" onClick={() => { haptic.selection(); setMode('practice'); }}
                    className={`p-4 rounded-[16px] border-2 text-sm font-bold transition-all ${mode === 'practice' ? 'bg-primary text-card border-primary shadow-brutal-sm' : 'bg-card text-secondary border-black/10'}`}
                  >
                    <div className="text-lg mb-1">📖</div>Practice Mode
                  </button>
                  <button type="button" onClick={() => { haptic.selection(); setMode('simulator'); }}
                    className={`p-4 rounded-[16px] border-2 text-sm font-bold transition-all ${mode === 'simulator' ? 'bg-primary text-card border-primary shadow-brutal-sm' : 'bg-card text-secondary border-black/10'}`}
                  >
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`px-3 py-3 rounded-[14px] border-2 text-sm font-bold transition-all text-left active:scale-95 ${
                        subject === s.id
                          ? 'bg-primary text-card border-primary shadow-brutal-sm'
          ...
          {setupModalType === 'notes' && (
            <div className="space-y-5">
              ...
              <div className="p-4 rounded-[20px] bg-purple-500/10 border-2 border-purple-500/20 flex items-center gap-3">
        ...
        <button onClick={handleStart} className="shrink-0 w-full btn-brutal py-4 bg-primary text-card rounded-[16px] font-black text-lg flex items-center justify-center gap-2 focus-ring hover:opacity-90 active:scale-95 transition-all">
