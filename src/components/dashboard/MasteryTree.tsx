    ...
    <div className="flex-1 flex flex-col pt-4 px-4 pb-24 min-h-screen overflow-y-auto animate-fade-in bg-ground custom-scrollbar" style={{ backgroundImage: 'radial-gradient(circle, #1B3A6B08 1px, transparent 1px)', backgroundSize: '20px 20px' }}>
      ...
      <div className="grid grid-cols-2 gap-3 mb-8">
        <div className="bg-card border border-black/5 shadow-sm rounded-[20px] p-5 flex flex-col justify-between">
          <span className="text-[10px] font-black text-tertiary uppercase tracking-wider mb-2 flex items-center gap-1.5"><Target className="w-3.5 h-3.5"/> Accuracy</span>
          <span className="text-4xl font-black text-primary tracking-tighter">{overallAccuracy}%</span>
        </div>
      ...
      <h2 className="text-[15px] font-black text-primary mb-4 uppercase tracking-wider flex items-center gap-2">
      ...
      {stats.length === 0 ? (
        <div className="bg-card border-2 border-dashed border-black/10 rounded-[24px] p-8 flex flex-col items-center text-center">
          <Brain className="w-12 h-12 text-tertiary opacity-30 mb-4" />
          <h3 className="text-[17px] font-black text-primary mb-1">Tree is Empty</h3>
          <p className="text-[13px] font-bold text-tertiary">Take an exam or quick practice to start leveling up your scholar tree!</p>
            ...
            return (
              <div key={stat.subject} className="bg-card border border-black/5 shadow-sm rounded-[20px] p-5 relative overflow-hidden group">
                <div className="flex justify-between items-start mb-4 relative z-10">
                  <div>
                    <h3 className="text-[15px] font-black text-primary leading-tight line-clamp-1">{stat.subject}</h3>
                    <div className="flex gap-2 items-center mt-1.5">
                      <span className="text-[10px] font-black bg-primary/10 text-primary px-2.5 py-0.5 rounded-full uppercase tracking-widest border border-primary/20">Lvl {level}</span>
                      <span className="text-[11px] font-bold text-tertiary">{accuracy}% Win Rate</span>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="text-2xl font-black text-primary tracking-tighter block leading-none">{stat.questions_correct}</span>
                    <span className="text-[9px] font-bold text-tertiary uppercase tracking-widest mt-1 block">Correct</span>
                  </div>
                </div>

                {/* Premium Progress Bar */}
                <div className="mt-1 relative z-10">
                  <div className="flex justify-between text-[10px] font-black text-tertiary uppercase tracking-widest mb-1.5">
