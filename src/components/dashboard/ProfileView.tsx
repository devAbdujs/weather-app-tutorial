  ...
  return (
    <div className="flex-1 flex flex-col pt-6 px-4 pb-24 overflow-y-auto animate-fade-in bg-ground custom-scrollbar">
      ...
      <div className="flex flex-col items-center mb-8 relative">
        ...
        <p className="text-xs font-bold text-tertiary uppercase tracking-widest mt-1">ID: {profile.telegram_id}</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-card border border-black/5 shadow-sm rounded-[24px] p-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`w-12 h-12 rounded-[16px] flex items-center justify-center ${isPremium ? 'bg-accent-gold text-primary shadow-sm' : 'bg-black/5 text-secondary'}`}>
              <Zap className={`w-6 h-6 ${isPremium ? 'fill-primary' : ''}`} />
            </div>
            <div>
              <h2 className="text-[16px] font-black text-primary">Subscription</h2>
              <p className="text-[13px] font-bold text-tertiary">{isPremium ? 'Premium Active' : 'Free Tier'}</p>
            </div>
          </div>
          {!isPremium && (
            <button className="px-5 py-2.5 bg-primary text-white rounded-[12px] text-[13px] font-black shadow-md active:scale-95 transition-all">
              Upgrade
            </button>
          )}
        </div>
        ...
        {/* Exam Goal Card */}
        <button onClick={handleRetakeOnboarding} className="w-full bg-card border border-black/5 shadow-sm rounded-[24px] p-5 flex items-center justify-between active:scale-95 transition-all text-left">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-[16px] bg-[#229ED9]/10 text-[#229ED9] flex items-center justify-center">
              <Target className="w-6 h-6" />
            </div>
            <div>
              <h2 className="text-[16px] font-black text-primary">Academic Goal</h2>
              <p className="text-[13px] font-bold text-tertiary capitalize line-clamp-1">{profile.target_exam} • {profile.stream}</p>
            </div>
          </div>
        ...
        <div className="bg-card border border-black/5 shadow-sm rounded-[24px] p-5 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-ground border border-black/5 rounded-[16px] p-3 text-center flex flex-col items-center justify-center h-20">
              <span className="text-xl font-black text-primary block leading-none mb-1">{profile.daily_streak}</span>
              <span className="text-[9px] font-bold text-tertiary uppercase tracking-widest">Streak</span>
            </div>
            <div className="bg-ground border border-black/5 rounded-[16px] p-3 text-center flex flex-col items-center justify-center h-20">
              <span className="text-xl font-black text-primary block leading-none mb-1">{totalQuestions}</span>
              <span className="text-[9px] font-bold text-tertiary uppercase tracking-widest">Drills</span>
            </div>
        ...
        {stats.length > 0 && (
          <div className="bg-card border border-black/5 shadow-sm rounded-[24px] p-5 flex flex-col gap-4">
            <h2 className="text-[15px] font-black text-primary flex items-center gap-2">
              <BarChart2 className="w-5 h-5 text-primary" /> Subject Mastery
            </h2>
                ...
                return (
                  <div key={s.subject} className="flex items-center justify-between border-b border-black/5 last:border-0 pb-3 last:pb-0">
                    <div className="flex flex-col">
                      <span className="text-[14px] font-black text-primary">{s.subject}</span>
                      <span className="text-[11px] font-bold text-tertiary">{s.questions_attempted} Qs Attempted</span>
                    </div>
                    <div className="text-right">
                      <span className="text-[15px] font-black text-primary block">{acc}%</span>
                    </div>
                  </div>
                );
              })}
