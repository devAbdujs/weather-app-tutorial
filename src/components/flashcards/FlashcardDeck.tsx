...
export const FlashcardDeck: React.FC<FlashcardDeckProps> = ({ subject }) => {
  ...
  return (
    <div className="min-h-screen bg-ground text-gray-900 dark:text-gray-100 pb-24 max-w-md mx-auto flex flex-col justify-between font-sans overflow-hidden">
      <header className="bg-ground/90 backdrop-blur-md border-b-2 border-black/5 dark:border-white/10 flex flex-col z-10 relative animate-fade-up">
        
        {/* Top Header */}
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center gap-3">
            {onExit && (
              <button onClick={() => { haptic.impact('light'); onExit(); }} className="w-10 h-10 flex items-center justify-center rounded-[12px] bg-card border border-black/5 dark:border-white/10 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 dark:text-gray-100 transition-colors focus-ring active:scale-[0.98] active:opacity-80">
                <ChevronLeft className="w-6 h-6" />
              </button>
            )}
            <div className="w-10 h-10 rounded-[12px] bg-primary/10 border border-black/5 dark:border-white/10 flex items-center justify-center shadow-sm">
              <Zap className="w-5 h-5 text-gray-900 dark:text-gray-100" />
            </div>
            <div>
          ...
          <button onClick={() => { haptic.impact('light'); loadDeck(); }} className="w-10 h-10 rounded-[12px] bg-card border border-black/5 dark:border-white/10 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 dark:text-gray-100 transition-colors focus-ring active:scale-[0.98] active:opacity-80 flex justify-center items-center shadow-sm">
            <Shuffle className="w-4 h-4" />
          </button>
        </div>

        {/* Progress Bar */}
        <div className="w-full h-1.5 bg-black/5 dark:bg-white/5">
          <div 
        ...
        {loading ? (
          ...
          <div className="w-full max-w-sm bg-card border border-primary/20 rounded-[32px] p-8 text-center space-y-6 shadow-lg animate-scale-bounce">
            <div className="w-20 h-20 rounded-[20px] bg-accent-emerald border border-primary/20 mx-auto flex items-center justify-center shadow-sm">
              <CheckCircle2 className="w-10 h-10 text-gray-900 dark:text-gray-100" />
            </div>
            <div>
              <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 tracking-tight">Deck Completed!</h2>
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400 mt-2">Excellent spaced repetition session.</p>
            </div>
          ...
          <div className="w-full max-w-sm space-y-6 relative flex flex-col items-center">
            ...
            <div className="w-full flex justify-between font-bold text-xs text-gray-500 dark:text-gray-400 tracking-widest uppercase px-1">
            ...
            >
              <div className={`relative w-full h-full duration-[500ms] transform-style-3d ease-snap ${isFlipped ? 'rotate-y-180' : ''}`}>
                ...
                <div className="absolute inset-0 w-full h-full bg-card border border-primary/20 rounded-[32px] p-8 flex flex-col justify-center items-center backface-hidden shadow-md">
                  <span className="absolute top-6 left-6 text-[10px] uppercase tracking-widest font-black text-gray-600 dark:text-gray-400 flex items-center gap-1.5"><HelpCircle className="w-4 h-4"/> Concept</span>
