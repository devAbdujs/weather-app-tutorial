  ...
  return (
    <div>
      <header className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight">Manage Admins</h1>
          <p className="text-gray-500 font-medium mt-1">Create accounts for content creators or read-only viewers.</p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-2 space-y-4">
          <h2 className="text-xl font-bold text-gray-900">Active Accounts</h2>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-ground/50 border-b-2 border-primary/10">
                  <th className="p-4 font-bold text-gray-500 uppercase tracking-wider text-xs">Username</th>
                  <th className="p-4 font-bold text-gray-500 uppercase tracking-wider text-xs">Role</th>
                  <th className="p-4 font-bold text-gray-500 uppercase tracking-wider text-xs">Created</th>
                </tr>
              </thead>
              <tbody>
                {admins.map((a: any) => (
                  <tr key={a.id} className="border-b border-primary/5 hover:bg-ground/50 transition-colors">
                    ...
                    <td className="p-4">
                      <span className={`px-2 py-1 rounded-lg text-xs font-bold border ${
                        ...
                        'bg-tertiary/10 text-gray-500 border-tertiary/20'
                      }`}>
                        {a.role.toUpperCase()}
                      </span>
                    </td>
